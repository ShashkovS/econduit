(function(){
        ...
        $.ajax({
            ...
            success: function(Response){
                        ...
                        for(var i = 0, l = Response.length; i < l; i++) {
                            ...
                            window.scroll(0,$(this).closest('li').position().top); 
    ...
    // Вывести подсказку
    function Help() {
        term.echo("[[;orange;black]Синтаксис: школьник листок задачи {стереть}  (или откат)]");
        ...
        if (selected_pupil_id != -1 && selected_list_id != -1 && selected_prob_ids.length > 0) {
            term.push(function(command) {
                if (command.match(/д|да/i)) {
                    ...
                    Request.List = selected_list_id;
                    SendRequest('update', Request);
                    term.pop();
                } else if (command.match(/н|не|нет/i)) {
                    term.pop();
                }
            }, {
    ...
    this.init = function(pupil_array, list_array, problem_array) {
        term = $('#coduit_terminal').terminal(function(command, term) {
            ...
            }, { prompt: 'conduit> ', name: 'Conduit', greetings: ''});
    }
}
